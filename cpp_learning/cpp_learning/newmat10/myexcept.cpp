...
void BaseException::AddMessage(const char* a_what)
{
   if (a_what)
   {
      ...
      if (l < r) { strcpy_s(what_error+SoFar, sizeof(what_error)/sizeof(what_error[0]) + 1, a_what); SoFar += l; }
      else if (r > 0)
      {
         strncpy_s(what_error+SoFar, sizeof(what_error) / sizeof(what_error[0]) + 1, a_what, r);
