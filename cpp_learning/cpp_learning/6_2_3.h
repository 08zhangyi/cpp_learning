...
double option_price_call_american_binomial(const double& S, const double& X, const double& r, const double& y,
	const double& sigma, const double& time, const int& steps)
{
	...
	double uu = u * u;
	double d = 1.0 / u;
	double p_up = (exp((r-y)*(time/steps)) - d) / (u - d);
...
double option_price_put_american_binomial(const double& S, const double& X, const double& r, const double& y,
	const double& sigma, const double& time, const int& steps)
{
	double R = exp(r*(time / steps));
	double Rinv = 1.0 / R;
...
int mf()
{
	...
	double time = 1;
	int steps = 100;
	cout << "美式看涨期权的价格：" << option_price_call_american_binomial(S, X, r, y, sigma, time, steps) << endl;
	cout << "美式看跌期权的价格：" << option_price_put_american_binomial(S, X, r, y, sigma, time, steps) << endl;
	return 0;
}