...
double bonds_yield_to_maturity_discrete(const vector<double>& times, const vector<double>& cashflows, const double& bondprice)
{
...
int mf()
{
	...
	cout << "债券价格：" << B << endl;
	cout << "到期收益率：" << 2 * bonds_yield_to_maturity_discrete(times, cflows, B) << endl;
	return 0;
}