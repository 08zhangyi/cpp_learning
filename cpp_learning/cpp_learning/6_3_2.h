...
double option_price_call_american_finite_diff_explicit(const double& S, const double& X, const double& r, const double& sigma, const double& time,
...
double option_price_put_american_finite_diff_explicit(const double& S, const double& X, const double& r, const double& sigma, const double& time,
	const int& no_S_steps, const int& no_t_steps)
{
	double sigma_sqr = sigma * sigma;
	unsigned int M;
...
int mf()
{
	...
	int no_S_steps = 20;
	int no_t_steps = 11;
	cout << "欧式看涨期权的价格" << option_price_call_american_finite_diff_explicit(S, X, r, sigma, time, no_S_steps, no_t_steps) << endl;
	cout << "欧式看跌期权的价格" << option_price_put_american_finite_diff_explicit(S, X, r, sigma, time, no_S_steps, no_t_steps) << endl;
	return 0;
}