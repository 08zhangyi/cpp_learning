...
template <class T> InternalNode<T>::InternalNode(T* newObject, Node<T>* newNext) : object(newObject), next(newNext) 
{
}

template <class T> Node<T>* InternalNode<T>::insert(T* newObject)
{
...
template <class T> Node<T>* TailNode<T>::insert(T* object)
{
...
template <class T> Node<T>* HeadNode<T>::insert(T* object)
{
...
template <class T> void LinkedList<T>::insert(T* pObject)
{
	head->insert(pObject);
}

void myFunction(LinkedList<Robot>& listOfRobots);
...
void myFunction(LinkedList<Robot>& listOfRobots)
{
	...
	while (true)
	{
		std::cout << "\nHow old is your robot (0 to stop)? ";
		std::cin >> val;
		if (!val)
			break;
